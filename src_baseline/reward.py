...
def get_custom_reward(info, prev_info, done):
    """
    Reward shaping focused on GENERALIZATION.

    Main idea:
    1) Progress (Δx_pos) is the PRIMARY reward
    2) Coin reward is AUXILIARY
    3) Death is heavily penalized

    Args:
        info: dict from env.step()
        prev_info: dict from previous step
    ...
    # -------------------------------------------------
    # 1️ Progress reward (MOST IMPORTANT)
    # -------------------------------------------------
    delta_x = info.get("x_pos", 0) - prev_info.get("x_pos", 0)
    reward += 0.1 * delta_x   # scale to keep reward stable

    # -------------------------------------------------
    # 2️ Coin reward (small auxiliary signal)
    # -------------------------------------------------
    delta_coin = info.get("coins", 0) - prev_info.get("coins", 0)
    reward += 1.0 * delta_coin

    # -------------------------------------------------
    # 3️ Death penalty
    # -------------------------------------------------
    # If episode ends without reaching the flag, penalize
